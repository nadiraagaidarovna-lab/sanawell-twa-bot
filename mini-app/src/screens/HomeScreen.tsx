...
export default function HomeScreen() {
  ...
  // Срез Г консолидации (CLAUDE.md 4.3.1) — контрол смены языка и напоминаний прямо на
  // /checkin/, поверх уже существующих /api/language и /api/reminder-opt-in. С появлением
  // Шага 0 онбординга (Срез О3, ТЗ 6.2.1) язык теперь ВПЕРВЫЕ определяется там (авто по
  // Telegram + ручной выбор) — но сам этот блок остаётся постоянным, не убирается: Надира
  // подтвердила (10.09.2026), что язык должен быть сменить "в любой момент" и после
  // онбординга, не только один раз на Шаге 0.
