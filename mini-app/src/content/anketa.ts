...
// Тизер "Этапы жизни после 40" сразу после шага 4 (ТЗ 5.3.1/6.2.2) — просто карточка-
// анонс, без перехода в образовательный раздел (тот ещё не построен).
export const STEP4_TEASER: Record<Lang, { title: string; body: string }> = {
  ru: {
    title: 'Этапы жизни после 40',
    body: 'Перименопауза → менопауза → постменопауза — три этапа с разными задачами для мозга, костей, мышц и сердца. Подробнее — в разделе «Обучение» (скоро).',
  },
  kk: {
    title: '40 жастан кейінгі кезеңдер',
    body: 'Перименопауза → менопауза → постменопауза — ми, сүйек, бұлшықет және жүрек үшін әртүрлі міндеттері бар үш кезең. Толығырақ — «Білім алу» бөлімінде (жақында).',
  },
};
