...
const TEXT = {
  ru: {
    // Срез О3 (ТЗ v2.12, 6.2.1): полноценное приветствие теперь живёт в Mini App (Шаг 0,
    // WelcomeScreen.tsx), не в чате бота — здесь остаётся только короткая нейтральная фраза
    // перед кнопкой. Прежний многоабзацный текст ("дорогая подруга") отсюда убран целиком:
    // он дублировал новый экран приветствия и содержал формулировку "схожу с ума"
    // (стигматизирующая, запрещена CLAUDE.md) — оба были реальными, уже задеплоенными
    // проблемами, не только дублированием контента.
...
function startBot({
  ...
  bot.onText(/\/start/, (msg) => {
    ...
    // Язык ещё не выбран на этом этапе (авто-определение — первый экран самого Web App,
    // Срез О3, ТЗ 6.2.1), поэтому короткая фраза двуязычная, как и раньше. Полноценное
    // приветствие сюда больше не относится — оно на Шаге 0 внутри Mini App.
    const text = `${TEXT.ru.greeting}\n\n— — —\n\n${TEXT.kk.greeting}`;
